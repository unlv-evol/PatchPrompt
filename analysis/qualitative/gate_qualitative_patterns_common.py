...
(`dataset/processed/final_analysis_dataset.csv`) so that the curated evidence
...
- evaluator-facing copies under qualitative_examples/<gate>/
"""
...
DATASET_PATH = ROOT / "dataset" / "processed" / "final_analysis_dataset.csv"
RESULTS_ROOT = ROOT / "results" / "qualitative"
EXAMPLES_ROOT = ROOT / "qualitative_examples"
...
def write_csv_xlsx_pdf(pattern_df: pd.DataFrame, full_df: pd.DataFrame, gate: str, title: str) -> None:
    """Write gate-specific qualitative evidence artifacts to results and examples."""
    result_dir = RESULTS_ROOT / gate
    example_dir = EXAMPLES_ROOT / gate
    ...
    for path in [csv_path, xlsx_path, pdf_path, full_path]:

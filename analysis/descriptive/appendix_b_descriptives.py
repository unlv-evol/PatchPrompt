...
`dataset/processed/final_analysis_dataset.csv` file. These outputs complement the
...
procedures.
"""
    (root / "results" / "descriptive" / "appendix_b_summary.md").write_text(text, encoding="utf-8")
...
def run(root: Path) -> list[pd.DataFrame]:
    ...
    outdir = root / "results" / "descriptive"

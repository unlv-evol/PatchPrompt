...
from analysis.common import load_analysis_dataset, write_csv, write_latex_table, safe_logit_fit, format_effect
...
def run(root: Path):
    ...
    write_csv(out, root/"results/tables/gate0_generation_model.csv")
    wide=out.pivot(index="Variable", columns="Model", values="Formatted").reset_index()
    write_latex_table(wide, root/"paper/tables/gate0_generation_model.tex", "Gate 0: Logistic Regression Results for Code Generation", "tab:gate0")
    return out

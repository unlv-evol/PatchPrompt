...
def run(root:Path):
    ...
    write_csv(out, root/"results/tables/gate1_adoption_model.csv")
    wide=out.pivot(index="Variable", columns="Model", values="Formatted").reset_index()
    write_latex_table(wide, root/"paper/tables/gate1_adoption_model.tex", "Gate 1: Logistic Regression Results for Code Adoption", "tab:gate1")
    return out

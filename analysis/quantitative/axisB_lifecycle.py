...
from analysis.common import load_analysis_dataset, write_csv, write_latex_table, format_effect
...
def run(root:Path):
    ...
    write_csv(out, root/"results/tables/axisb_lifecycle_model.csv")
    wide=out.pivot(index="Variable", columns="Model", values="Formatted").reset_index()
    write_latex_table(wide, root/"paper/tables/axisb_lifecycle_model.tex", "Axis B: Cause-Specific Cox Results for PR Lifecycle", "tab:axisb")
    return out

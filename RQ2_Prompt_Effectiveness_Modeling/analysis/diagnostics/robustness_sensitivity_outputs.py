...
from RQ2_Prompt_Effectiveness_Modeling.analysis.common import ensure_dir, load_analysis_dataset, write_csv, write_latex_table, stars
...
def _build_prompt_tokens(root: Path, df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    pdf_paths = list((root / "Dataset_Construction" / "raw_data" / "chatgpt").rglob("*.pdf"))
    ...
    write_csv(out, root / "RQ2_Prompt_Effectiveness_Modeling" / "results" / "diagnostics" / "prompt_tokens.csv")
    d = df.copy()
...
def _make_memo(results: pd.DataFrame, exclusions: pd.DataFrame, failures: pd.DataFrame, stability: pd.DataFrame, reuse_counts: pd.DataFrame, root: Path) -> str:
    top_repos = pd.read_csv(root / "RQ2_Prompt_Effectiveness_Modeling" / "results" / "diagnostics" / "top_repository_counts.csv")
    top_langs = pd.read_csv(root / "RQ2_Prompt_Effectiveness_Modeling" / "results" / "diagnostics" / "top_language_counts.csv")
    prompt_tokens = pd.read_csv(root / "RQ2_Prompt_Effectiveness_Modeling" / "results" / "diagnostics" / "prompt_tokens.csv")
...
def run(root: Path):
    outdir = ensure_dir(root / "RQ2_Prompt_Effectiveness_Modeling" / "results" / "diagnostics")
    ...
    paper_dir = root / "RQ2_Prompt_Effectiveness_Modeling" / "results" / "tables_tex"
